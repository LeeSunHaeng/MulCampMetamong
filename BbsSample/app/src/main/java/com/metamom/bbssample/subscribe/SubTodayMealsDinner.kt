        ...
        *  2-1) 불러온 식단 정보는 식단 추천 이력 확인을 위하여 REMEBER TABLE에 저장 */
        ...
        // 2) 식단 추천 이력이 없을 경우
            ...
            if (SubTodayMealSingleton.type == 0) {
                ...
                // 2-ii) 다이어트 식단 가져오기
                todayMeal = SubscribeDao.getInstance().subRandomDietMeal(SubDietMealDto(0, SubTodayMealSingleton.time!!, "", "", SubTodayMealSingleton.dinnerKcal!!.toDouble(), 0, SubTodayMealSingleton.type.toString(), MemberSingleton.id))
            ...
            // 2-i) [운동] 회원이 운동 식단을 신청하였을 경우 (type == 1)
                ...
                // 2-ii) 운동 식단 가져오기
                todayMeal = SubscribeDao.getInstance().subRandomExerMeal(SubExerMealDto(0, SubTodayMealSingleton.time!!, "", "", SubTodayMealSingleton.dinnerKcal!!.toDouble(), 0, SubTodayMealSingleton.type.toString(), MemberSingleton.id))
                // 2-iv) 추천한 식단 정보 DB TABLE에 저장하기
                if (todayMeal != null) {
